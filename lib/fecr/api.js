...
class Api {
  constructor(api) {
    ...
    if (this.environment === "prod") {
      ...
      this.logoutEndpoit = "https://idp.comprobanteselectronicos.go.cr/auth/realms/rut/protocol/openid-connect/logout";
    } else {
      this.clientId = "api-stag";
      this.vouchersEndpoint = "https://api-sandbox.comprobanteselectronicos.go.cr/recepcion/v1/";
      this.authEndpoint = "https://idp.comprobanteselectronicos.go.cr/auth/realms/rut-stag/protocol/openid-connect/token";
      this.logoutEndpoit = "https://idp.comprobanteselectronicos.go.cr/auth/realms/rut-stag/protocol/openid-connect/logout";
    }
  }
    ...
    return new Promise((resolve, reject) => {
      if (this.refresh) {
        if (this.expires < Date.now() && this.refreshExpires > Date.now()) {
          ...
          this.expires < Date.now() && this.refreshExpires < Date.now()
        ) {
          this.newToken()
              .then(() => {
                resolve(this.token);
    ...
    return new Promise((resolve, reject) => {
      this.getToken()
          .then((token) => axios.post(
              `${this.vouchersEndpoint}recepcion`,
    ...
    return new Promise((resolve, reject) => {
      this.getToken()
          .then((token) => axios.get(
              `${this.vouchersEndpoint}recepcion/${key}`,
